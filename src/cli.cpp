...
std::string vm_description(const std::string& vm_brand) {
    ...
    std::map<std::string, const char*> description_table{
        ...
        { VM::brands::HYPERV_ARTIFACT, "The CLI detected Hyper-V operating as a Type 1 hypervisor (bare-metal), not as a guest virtual machine. While hardware/firmware signatures match Microsoft's Hyper-V architecture by design, our \"Hyper-X\" mechanism differentiates between the root partition (host OS) and guest VM environments. This prevents false positives where Hyper-V artifacts originate from the host OS or nested virtualization frameworks, rather than an actual Hyper-V guest VM." },         
