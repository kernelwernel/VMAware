...
std::string vm_description(const std::string& vm_brand) {
    ...
    std::map<std::string, const char*> description_table{
        ...
        { VM::brands::VMWARE_HARD, "VMware Hardware-Assisted Virtualization refers to optimizations leveraging Intel VT-x/AMD-V CPU extensions in products like ESXi and Workstation. Enhances performance by reducing binary translation overhead for privileged instructions, particularly beneficial for 64-bit guests and Windows Hyper-V enlightened VMs." },
        ...
        { VM::brands::QEMU_KVM_HYPERV, "QEMU-KVM with Hyper-V extensions combines QEMU's device emulation with Hyper-V's paravirtual clock, synthetic interrupts, and VP index support. This configuration optimizes Windows guest performance on Linux hosts by reducing exit latency and improving timer precision through Hyper-V enlightenment interfaces." },
        ...
        { VM::brands::HYPERV_VPC, "Hyper-V Virtual PC compatibility layer maintains support for legacy Virtual PC 2007 configurations, including VHD disk formats and VM additions. Primarily used for migrating older Windows XP/Vista VMs to modern Hyper-V environments while preserving backward compatibility with legacy virtualization tools." },
        { VM::brands::MSXTA, "MSXTA (Microsoft Xen Technology Adaptation) was an experimental project (2005-2007) that integrated Xen hypervisor components with Windows Server. Explored paravirtualization concepts later incorporated into Hyper-V's enlightenments, though never officially released due to architectural differences between Xen's and Microsoft's virtualization approaches." },
        ...
        { VM::brands::ACRN, "ACRN (Adaptive Cloud Resource Nexus) is Intel's open-source type 1 hypervisor optimized for IoT edge devices. Features hybrid real-time capabilities with split architecture: Service VM handles I/O while User VMs run safety-critical workloads. Supports automotive scenarios like digital clusters and IVI systems on Apollo Lake platforms." },
        { VM::brands::QNX, "QNX Hypervisor is a real-time virtualization platform for embedded systems, enabling concurrent execution of QNX Neutrino RTOS and Linux/Android on ARM/x86. Provides time partitioning with <1μs interrupt latency for automotive systems, certified to ISO 26262 ASIL D safety standards. Used in Audi MIB3 and BMW iDrive systems." },
        { VM::brands::HYBRID, "Hybrid virtualization combines multiple techniques like containers (Docker) and hypervisors (KVM) through projects like Kata Containers. Creates lightweight VMs that behave like containers, offering hardware isolation while maintaining container-like density. Used in cloud-native environments requiring strict security boundaries between workloads." },
        { VM::brands::SANDBOXIE, "Sandboxie is an application sandbox for Windows that redirects file/registry writes to virtualized storage. Acquired by Sophos in 2019 and open-sourced in 2020, it uses kernel-mode drivers (SbieDrv.sys) to isolate processes without full VM overhead. Commonly used for testing untrusted software or browsing securely." },
        ...
        { VM::brands::ANUBIS, "Anubis Sandbox is a malware analysis platform that executes suspicious files in instrumented VMs/containers. Tracks API calls, registry changes, and network patterns using kernel drivers and hooking techniques. Generates detailed JSON reports with IOC extraction, used by cybersecurity teams for threat intelligence." },
        ...
        { VM::brands::CWSANDBOX, "CWSandbox (Comodo Valkyrie) is a cloud-based threat analysis system using kernel-mode instrumentation. Employs machine learning to detect zero-day exploits through API call sequencing and network pattern analysis. Integrated with Comodo's endpoint protection for automated threat blocking based on sandbox verdicts." },
        ...
        { VM::brands::UNISYS, "Unisys ClearPath Forward is a mainframe virtualization platform combining custom CMOS/ASIC chips with Intel Xeon processors. Supports secure partitioning (s-PAR) for legacy OS 2200/MCP workloads, offering binary compatibility for 1960s-era Univac applications. Used in government systems requiring EAL4+ certification and air-gapped security." },
        ...
        { VM::brands::INTEL_KGT, "Intel Kernel Guard Technology (KGT) was a research hypervisor for Windows kernel protection. Demonstrated secure enclaves for critical OS components using VT-x/EPT before being superseded by CET (Control-flow Enforcement Technology) and HyperGuard in Windows 10." },
        ...
        { VM::brands::MINIVISOR, "MiniVisor is a lightweight ARM hypervisor for Cortex-A/R real-time systems. Implements secure world isolation for TrustZone applications, providing memory protection and interrupt virtualization. Used in automotive ECUs and industrial PLCs requiring mixed-criticality workloads." },
